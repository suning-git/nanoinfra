"""run.py — train both arms (modern GPT vs GPT-2-style trunk) and collect their
val-loss trajectories. Same model size / data / budget / recipe; the ONLY
difference is the trunk. Mirrors the exemplar's scaling.py driving pattern:
subprocess the blessed orchestrator, parse the scheduled val evals from its log.
...
Needs a FineWeb shard on disk (outputs/base_data/) — fetch a couple first:
    ...
    python projects/example_gpt2_vs_modern/run.py      # trains both arms
